...
def load_data(source_language, target_language):
    ...
    target_texts_formal = []
    with open(base_path + "/formality-control.train.topical_chat." + language_pair + ".informal." + target_language, "r") as f:
        for line in f:
            target_texts_formal.append(line.strip())

    feminine_texts_file = base_path + "/formality-control.train.topical_chat." + language_pair + ".informal.feminine." + target_language
    if os.path.exists(feminine_texts_file): 
        target_texts_formal_feminine = []
        with open(base_path + "/formality-control.train.topical_chat." + language_pair + ".informal.feminine." + target_language, "r") as f:
            for line in f:
